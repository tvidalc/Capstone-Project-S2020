...
namespace gp {
    ...
    void OnGPIS::train(const vecNode3 &samples) {
        ...
        if (N > 0) {
            ...
            for (auto it = samples.begin(); it != samples.end(); it++, k++) {
                ...
                siggrad(k) = (*it)->getGradNoise();
                if (siggrad(k) > 0.1001 ||
                    (fabs(grad(0, k)) < 1e-6 && fabs(grad(1, k)) < 1e-6 && fabs(grad(2, k)) < 1e-6)) {
                    gradflag[k] = 0.0;
                    sigx(k) = 2.0;
                } else {
                    gradflag[k] = 1.0;
    ...
    void OnGPIS::test2Dpoint(const EVectorX &xt, float &val, float &gradx, float &grady, float &varval, float &vargradx,
                             float &vargrady) {
        if (!isTrained()) {
            return;
        }
    ...
    void OnGPIS::test2Dpoint(float px, float py, float &val, float &gradx, float &grady, float &varval, float &vargradx,
                             float &vargrady) {
        if (!isTrained()) {
            return;
        }
