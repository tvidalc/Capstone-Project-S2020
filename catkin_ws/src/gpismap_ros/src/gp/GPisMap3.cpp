        ...
        for (int n_ = 0; n_ < n; n_++) {
            ...
            for (int m_ = 0; m_ < m; m_++) {
                ...
                if ((k < N) && isRangeValid(dataz[k])) {
                    ...
                    obs_valid_xyzglobal.push_back(
                            pose_R[0] * xloc + pose_R[3] * yloc + pose_R[6] * dataz[k] + pose_tr[0]);
                    obs_valid_xyzglobal.push_back(
                            pose_R[1] * xloc + pose_R[4] * yloc + pose_R[7] * dataz[k] + pose_tr[1]);
                    obs_valid_xyzglobal.push_back(
                            pose_R[2] * xloc + pose_R[5] * yloc + pose_R[8] * dataz[k] + pose_tr[2]);
                    obs_numdata++;
                } else {
        ...
        if (t != 0 && gpo != 0) {
            ...
            if (oc.size() > 0) {
                ...
                for (auto it = oc.begin(); it != oc.end(); it++, k++) {
                    ...
                    float sqr_range =
                            (ct.x - pose_tr[0]) * (ct.x - pose_tr[0]) + (ct.y - pose_tr[1]) * (ct.y - pose_tr[1]) +
                            (ct.z - pose_tr[2]) * (ct.z - pose_tr[2]);
                    ...
                    for (auto it_ = ext.begin(); it_ != ext.end(); it_++) {
                        float x_loc = pose_R[0] * ((*it_).x - pose_tr[0]) + pose_R[1] * ((*it_).y - pose_tr[1]) +
                                      pose_R[2] * ((*it_).z - pose_tr[2]);
                        float y_loc = pose_R[3] * ((*it_).x - pose_tr[0]) + pose_R[4] * ((*it_).y - pose_tr[1]) +
                                      pose_R[5] * ((*it_).z - pose_tr[2]);
                        float z_loc = pose_R[6] * ((*it_).x - pose_tr[0]) + pose_R[7] * ((*it_).y - pose_tr[1]) +
                                      pose_R[8] * ((*it_).z - pose_tr[2]);
                        if (z_loc > 0) {
                            float xv = x_loc / z_loc;
                            float yv = y_loc / z_loc;

                            within_angle = int(
                                    (xv > u_obs_limit[0]) && (xv < u_obs_limit[1]) && (yv > v_obs_limit[0]) &&
                                    (yv < v_obs_limit[1]));
                        }
    ...
    void GPisMap3::reEvalPoints(std::vector<std::shared_ptr<Node3> > &nodes) {
        ...
        for (auto it = nodes.begin(); it != nodes.end(); it++) {
            ...
            float x_loc = pose_R[0] * (pos.x - pose_tr[0]) + pose_R[1] * (pos.y - pose_tr[1]) +
                          pose_R[2] * (pos.z - pose_tr[2]);
            float y_loc = pose_R[3] * (pos.x - pose_tr[0]) + pose_R[4] * (pos.y - pose_tr[1]) +
                          pose_R[5] * (pos.z - pose_tr[2]);
            float z_loc = pose_R[6] * (pos.x - pose_tr[0]) + pose_R[7] * (pos.y - pose_tr[1]) +
                          pose_R[8] * (pos.z - pose_tr[2]);
            ...
            float norm_grad_new = std::sqrt(grad_new_loc.x * grad_new_loc.x + grad_new_loc.y * grad_new_loc.y +
                                            grad_new_loc.z * grad_new_loc.z);
            ...
            float view_ang = std::max(
                    -(x_new[0] * grad_new_loc.x + x_new[1] * grad_new_loc.y + x_new[2] * grad_new_loc.z) / dist,
                    (float) 1e-1);
            ...
            if (grad_noise_old > 0.5 || grad_noise_old > 0.6) { ;
                ...
                float dist = 0.5 * std::sqrt(
                        (pos.x - pos_new.x) * (pos.x - pos_new.x) + (pos.y - pos_new.y) * (pos.y - pos_new.y) +
                        (pos.z - pos_new.z) * (pos.z - pos_new.z));
                ...
                // Noise update
                grad_noise = std::min((float) 1.0, std::max(grad_noise * grad_noise_old / grad_noise_sum + dist,
                                                            setting.map_noise_param));
    ...
    void GPisMap3::evalPoints() {
        ...
        for (; k < obs_numdata; k++) {
            ...
            if (norm_grad > 1e-6) {
                ...
                float dist = std::sqrt(obs_valid_xyzlocal[k3] * obs_valid_xyzlocal[k3] +
                                       obs_valid_xyzlocal[k3 + 1] * obs_valid_xyzlocal[k3 + 1] +
                                       obs_valid_xyzlocal[k3 + 2] * obs_valid_xyzlocal[k3 + 2]);
                ...
                float view_ang = std::max(
                        -(obs_valid_xyzlocal[k3] * grad_loc_x + obs_valid_xyzlocal[k3 + 1] * grad_loc_y +
                          obs_valid_xyzlocal[k3 + 2] * grad_loc_z) / dist, (float) 1e-1);
                float view_ang2 = view_ang * view_ang;
                float view_noise = setting.min_position_noise * ((1.0 - view_ang2) / view_ang2);
                noise += view_noise;
            }
                ...
                if (res[k8 + 4] > var_thre) {
                    ...
                    if (need_wsum) {
                        ...
                        std::sort(std::begin(idx), std::end(idx),
                                  [&](int i1, int i2) { return var2[i1 * 4] < var2[i2 * 4]; });
