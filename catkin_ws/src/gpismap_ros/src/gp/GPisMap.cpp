    ...
    bool GPisMap::preproData(float *datax, float *dataf, int N, std::vector<float> &pose) {
        ...
        for (int k = 0; k < N; k++) {
            ...
            if (isRangeValid(dataf[k])) {
                ...
                obs_xyglobal.push_back(
                        pose_R[0] * xloc + pose_R[2] * yloc + pose_tr[0]); // indices based on the matlab convention
    ...
    void GPisMap::evalPoints() {
        ...
        for (int k = 0; k < obs_numdata; k++) {
            ...
            if (norm_grad > 1e-6) {
                ...
                float view_ang = std::max(-(obs_xylocal[k2] * grad_loc_x + obs_xylocal[k2 + 1] * grad_loc_y) / dist,
                                          (float) 1e-1);
                float view_ang2 = view_ang * view_ang;
                float view_noise = setting.min_position_noise * ((1.0 - view_ang2) / view_ang2);
                noise += view_noise;
                ...
                if (res[k6 + 3] > var_thre) {
                    ...
                    if (need_wsum) {
                        ...
                        std::sort(std::begin(idx), std::end(idx),
                                  [&](int i1, int i2) { return var2[i1 * 3] < var2[i2 * 3]; });
